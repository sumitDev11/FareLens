...
import { getModelInfo, getMarketAnalytics, listWatchlists, listSavedSearches, listAlerts, FLIGHT_OPTIONS } from '../api/client';
...
const StatCard = ({ title, value, icon: Icon, trend, trendLabel, colorClass }) => (
    <div className="bg-surface border border-border rounded-xl p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
        <div className="flex justify-between items-start mb-4">
            ...
            <div className={`flex items-center justify-center w-8 h-8 rounded-lg bg-bg-secondary border border-border text-text-tertiary transition-colors ${colorClass}`}>
                <Icon size={16} />
            </div>
        </div>
        <div className="flex items-baseline gap-3">
            ...
            {trend && (
                <div className={`flex items-center text-xs font-semibold px-1.5 py-0.5 rounded-md ${trend.includes('▲') || trend.includes('+') ? 'text-success bg-success/10' : 'text-error bg-error/10'}`}>
                    {trend}
                </div>
            )}
        </div>
...
const ShortcutCard = ({ title, icon: Icon, items, type, onClickViewAll, emptyText }) => {
    return (
        <div className="bg-surface border border-border rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] flex flex-col h-[280px]">
            <div className="p-5 border-b border-border flex justify-between items-center shrink-0">
                ...
                <button onClick={onClickViewAll} className="text-xs font-medium text-text-secondary hover:text-primary transition-colors flex items-center gap-1">
            ...
            <div className="p-3 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2">
                {items && items.length > 0 ? (
                    items.map((item, i) => (
                        <div key={i} className="p-3 rounded-lg bg-bg-secondary border border-border hover:border-text-tertiary transition-colors flex flex-col gap-1.5 group cursor-default">
                            {type === 'watchlist' && (
                                <>
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-1.5 text-sm font-semibold text-text-primary">
                                            {item.source_city} <ArrowRight size={12} className="text-text-tertiary" /> {item.destination_city}
                                        </div>
                                    </div>
                                    <p className="text-xs text-text-secondary">Tracked since {new Date(item.created_at).toLocaleDateString()}</p>
                                </>
                            )}
                            {type === 'search' && (
                                <>
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-[10px] uppercase font-bold text-text-tertiary bg-surface px-1.5 py-0.5 rounded border border-border">{item.flight_class}</span>
                                    </div>
                                    <p className="text-xs text-text-secondary">Departs {new Date(item.travel_date).toLocaleDateString()}</p>
                                </>
                            )}
                            {type === 'alert' && (
                                <>
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border uppercase ${item.status === 'triggered' ? 'bg-error/10 text-error border-error/20' : 'bg-success/10 text-success border-success/20'}`}>
                                            {item.status}
                                        </span>
                                    </div>
                                    <p className="text-xs text-text-secondary">Target: ₹{item.target_price.toLocaleString('en-IN')}</p>
                                </>
                            )}
                        </div>
                    ))
        ...
        return (
            <div className="flex flex-col gap-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[...Array(4)].map((_, i) => (
                        <div key={i} className="rounded-xl border border-border bg-surface animate-pulse" style={{ height: 140 }} />
                    ))}
                </div>
            </div>
        );
    ...
    return (
        <div className="flex flex-col gap-8">
            <div className="flex justify-between items-end mb-2 flex-col lg:flex-row gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-text-primary mb-1 tracking-tight">Overview</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-surface border border-border rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-base font-semibold text-text-primary tracking-tight">Average Fare by Class</h3>
                    </div>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={avgPriceChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                    ...
                                    <Tooltip
                                        ...
                                        contentStyle={{ backgroundColor: '#0F172A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' }}
                                        itemStyle={{ color: '#F8FAFC', fontSize: '13px', fontWeight: 600 }}
                                        formatter={(value) => [`₹${value.toLocaleString('en-IN')}`, 'Average Price']}
                                        labelStyle={{ color: '#94A3B8', fontSize: '12px', marginBottom: '4px' }}
                                    />
                                    <Bar dataKey="price" radius={[6, 6, 0, 0]} maxBarSize={50} activeBar={{ fill: '#38BDF8' }}>
                    ...
                    <form onSubmit={handlePredict} className="flex-1 flex flex-col gap-4">
                        <div>
                            ...
                            <select 
                                ...
                                onChange={e => setPredictForm({...predictForm, source_city: e.target.value})}
                            >
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                onChange={e => setPredictForm({...predictForm, destination_city: e.target.value})}
                            >
                                {FLIGHT_OPTIONS.cities.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                            </select>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <input 
                                    ...
                                    onChange={e => setPredictForm({...predictForm, travel_date: e.target.value})}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-text-secondary mb-1.5">Class</label>
                                <select 
                                    className="w-full px-3 py-2.5 rounded-lg text-sm bg-bg-secondary border border-border text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 appearance-none cursor-pointer"
                                    value={predictForm.flight_class}
                                    onChange={e => setPredictForm({...predictForm, flight_class: e.target.value})}
                                >
                                    {FLIGHT_OPTIONS.classes.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                                </select>
                            </div>
                        </div>
                        <button type="submit" className="mt-auto w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-sm font-semibold cursor-pointer transition-all duration-200 border-none bg-primary text-bg-primary shadow-[0_2px_10px_rgba(34,211,238,0.2)] hover:shadow-[0_4px_15px_rgba(34,211,238,0.4)] hover:-translate-y-0.5 hover:brightness-110">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-surface border border-border rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] flex flex-col h-[280px]">
                    <div className="p-5 border-b border-border flex justify-between items-center shrink-0">
                        ...
                        <button onClick={() => setActiveTab('market-analytics')} className="text-xs font-medium text-text-secondary hover:text-primary transition-colors flex items-center gap-1">
                    ...
                    <div className="p-3 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2">
                        {market && market.popular_routes.length > 0 ? (
                            market.popular_routes.slice(0, 4).map((route, i) => (
                                <div key={i} className="p-3 rounded-lg bg-bg-secondary border border-border hover:border-text-tertiary transition-colors flex justify-between items-center group cursor-default">
                                    <div className="flex items-center gap-2 text-sm font-semibold text-text-primary">
                                        <MapPin size={12} className="text-text-tertiary group-hover:text-warning" /> {route.source_city} <ArrowRight size={12} className="text-text-tertiary" /> {route.destination_city}
                                    </div>
                                    <span className="text-xs font-bold text-text-secondary">{route.interest_count} views</span>
                                </div>
                            ))
                        ) : (
                            <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                                <TrendingUp size={20} className="text-text-tertiary mb-2 opacity-50" />
